...
impl Display for TicketNewError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let message = match self {
            TicketNewError::EmptyTitle => "Title cannot be empty",
            TicketNewError::TooLongTitle => "Title cannot be longer than 50 bytes",
            TicketNewError::EmptyDescription => "Description cannot be empty",
            TicketNewError::TooLongDescription => "Description cannot be longer than 500 bytes",
        };
        write!(f, "{message}")
    }
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    result.unwrap_or_else(|e| {
        match e {
            TicketNewError::EmptyDescription | TicketNewError::TooLongDescription => {
